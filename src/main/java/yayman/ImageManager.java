...
public class ImageManager extends javax.swing.JDialog {
    ...
    private void formWindowClosing(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosing
        ...
        if (lblImage.getIcon() != null) {
            ((ImageIcon) lblImage.getIcon()).getImage().flush();
        }
        if (lblMidImage.getIcon() != null) {
            ((ImageIcon) lblMidImage.getIcon()).getImage().flush();
        }
    ...
    private void loadMovieDbPosters(Vector dbImages) {
        try {
            ...
            ArrayList<Artwork> artworks = getArtworkOfType(TMDb.getMovieImages(mdb.getId(), "").getResults());
    ...
    private void loadTvDbPosters(Vector dbImages) {
        ...
        if (tvMode.equals(BannerMode)) {
            bannerType = "seasonwide";
        }
    ...
    public void fitAttachToWindow() {
        ...
        threadPool.submit(new MySwingWorker() {
            @Override
            protected Void doInBackground() {
                if (lblAttachImage.getIcon() != null) {
                    ((ImageIcon) lblAttachImage.getIcon()).getImage().flush();
                }
                if (attachImage != null) {
                    int w = ImageManager.this.getWidth() - 155;
                    int h = ImageManager.this.getHeight() - 75;
    ...
    public void fitOtherToWindow() {
        ...
        threadPool.submit(new MySwingWorker() {
            @Override
            protected Void doInBackground() {
                if (lblOtherImage.getIcon() != null) {
                    ((ImageIcon) lblOtherImage.getIcon()).getImage().flush();
                }
                if (otherImage != null) {
                    //Image img = newImage;
    ...
    public void displayImageFromUrl() {
        if (!internetTxt.getText().endsWith(".png") && !internetTxt.getText().endsWith(".bmp") && !internetTxt.getText().endsWith(".jpg")) {
            return;
        }
        try {
            if (otherImage != null) {
            ...
            if (conn.getContentLength() > -1) {
                otherImage = new ImageIcon(new URL(internetTxt.getText())).getImage();
            }
        } catch (Exception ex) {
            logger.severe("Invalid URL to display image: " + internetTxt.getText());
            otherImage = null;
        }
        if (otherImage != null && otherImage.getWidth(null) == -1) {
            otherImage = null;
        }
        fitOtherToWindow();
    }
